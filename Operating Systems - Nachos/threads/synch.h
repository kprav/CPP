...
class Lock {
  ...
  private:
    ...
    enum LosckState {FREE,BUSY} lockState; //track the lock status either free or busy
    Thread *lockOwnerThread;               //thread that is owner of the lock
    List *lockWaitQueue;                   //a wait queue for the lock
	int howMany;
};
...
class Condition {
  public:
    ...
    void Broadcast(Lock *conditionLock);// the currentThread for all of 
					// these operations
    bool isSomebodyWaiting(void);

  private:
    char* name;
    // plus some other stuff you'll need to define
    List *condWaitQueue;                // a wait queue for the condition variable
